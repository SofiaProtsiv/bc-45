...
})({"jFY0e":[function(require,module,exports) {
...
const getFormData = ()=>{
    const data = {};
    for (const { name , value  } of formElements)if (name) data[name] = value;
    return data;
};
...
const getSavedFormData = ()=>{
    try {
        ...
        for (const element of formElements)// if(saveData[element.name]){}
        if (element.name in saveData) element.value = saveData[element.name];
        const message = "Form has been refilled with saved data!";
        (0, _notiflixDefault.default).Notify.info(message);
    } catch (error) {
        const message = "Enter data!";
        (0, _notiflixDefault.default).Notify.warning(message);
    }
};
...
},{}]},["jFY0e","l9hyy"], "l9hyy", "parcelRequire5c0a")
